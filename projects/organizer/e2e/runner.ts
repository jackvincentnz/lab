...
export default async function globalSetup() {
  ...
  try {
    ...
    cleanupTargets.push(startedNetwork);
    ...
    ]).then(([loadedProxy]) => startProxy(loadedProxy, startedNetwork));
    ...
    const baseURL = `http://127.0.0.1:${startedProxy.getFirstMappedPort()}`;
    await waitForProxy(baseURL);
    process.env["ORGANIZER_E2E_BASE_URL"] = baseURL;
...
async function startProxy(
  ...
  const environment: Environment = {
    NGINX_PORT: PROXY_PORT.toString(),
  ...
  return proxyContainer
    .withNetwork(network)
    .withExposedPorts(PROXY_PORT as PortWithOptionalBinding)
    .withEnvironment(environment)
    .withWaitStrategy(Wait.forHttp("/task", PROXY_PORT).forStatusCode(301))
    .start();
}
...
async function cleanup(targets: StartedResource[]) {
  for (const target of targets.reverse()) {
    try {
      ...
      console.error("Failed to stop E2E resource cleanly.", error);
    }
  }
}

async function waitForProxy(baseURL: string) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // Docker Desktop may briefly expose the mapped port before it can accept host traffic.
    }

    await new Promise((resolve) => setTimeout(resolve, 250));
  }

  throw new Error(`Organizer proxy was not reachable at ${baseURL}/task/`);
}
